/**
 * Kanahoma Post Carousel — behavior (isolated IIFE to avoid global collisions)
 */
(function(){

// Debug controls (set window.KANAHOMA_CAROUSEL_DEBUG = true to enable verbose logs)
const DEBUG = typeof window !== 'undefined' && !!(window.KANAHOMA_POST_CAROUSEL_DEBUG || window.KANAHOMA_CAROUSEL_DEBUG);
...
const findTrack = (root) => {
  // Post-carousel variant mounts on WP Post Template
  if (root.matches('[data-post-carousel], .kanahoma-post-carousel')) {
    const pt = root.querySelector('.wp-block-post-template');
    if (pt) return pt;
  }
  // Regular carousel variants
  return (
...
const initCarousel = (root) => {
  ...
  const slides = Array.from(items.children).filter((el) => el.nodeType === 1 && !el.matches('.kanahoma-carousel__items::before, .kanahoma-carousel__items::after'));
  ...
  // Equalize slide heights to tallest slide (handles Group-based layouts)
  ...
  const enableDrag = () => {
    ...
    const onDown = (e) => {
      ...
      // Do not initiate drag from interactive elements; allow native navigation
    ...
    const onUp = (e) => {
      ...
      if (moved) {
        ...
        if (ptrType === 'mouse') {
          ...
          let v = (e.clientX - lastX) / Math.max(1, (now - lastT));
          v *= 18;
          const stepInertia = () => {
            items.scrollLeft -= v;
            v *= 0.94;
  ...
  const firstLayout = () => {
    ...
    try {
      const style = getComputedStyle(root);
      const explicit = parseInt(style.getPropertyValue('--step'), 10);
      const step = Number.isFinite(explicit) && explicit > 0 ? explicit : (function(){
        const vw = window.innerWidth;
        const m = parseInt(style.getPropertyValue('--spv-mobile'), 10) || 1;
        const t = parseInt(style.getPropertyValue('--spv-tablet'), 10) || m;
        const d = parseInt(style.getPropertyValue('--spv-desktop'), 10) || t;
        return (vw >= 1024) ? d : (vw >= 700) ? t : m;
      })();
      const one = step === 1;
    ...
    updateButtons();
    updateProgress();
    // After initial layout, set equal heights and re-run when images load
    equalizeHeights();
    try {
  ...
  // Observe for content changes inside items (e.g., Query updates), then equalize
  try {
    const mo = new MutationObserver(() => { equalizeHeights(); });
    mo.observe(items, { childList: true, subtree: true });
  } catch (e) {}
};

const initAll = () => {
  // Only initialize post-carousel instances here
  const all = document.querySelectorAll('[data-post-carousel], .kanahoma-post-carousel');
  if (DEBUG) { try { console.info('[Kanahoma Carousel] found', all.length, 'carousels'); } catch (e) { } }
