...
<section <?php echo $wrapper_attributes; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
  ...
  <div class="kanahoma-carousel__viewport" data-carousel-viewport>
    ...
    <?php
      // For Query block: render inner blocks (Query block will output .wp-block-post-template)
      // JS will mount on .wp-block-post-template as the carousel track
          ...
          if ( ! empty( $inner_blocks ) ) {
            foreach ( $inner_blocks as $inner ) {
              if ( method_exists( $inner, 'render' ) ) {
                echo $inner->render(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
              } elseif ( isset( $inner->parsed_block ) ) {
                echo render_block( $inner->parsed_block ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
              }
            }
          }
        }
